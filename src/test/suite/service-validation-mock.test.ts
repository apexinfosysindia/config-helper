...
import { ApexServices } from "@apexinfosysindia/js-websocket";
...
// Mock ApexOS connection with predefined services
class MockHaConnection implements IHaConnection {
  private mockServices: ApexServices = {
    light: {
      turn_on: {
        name: "Turn on",
  ...
  async getHassServices(): Promise<ApexServices> {
...
suite("Action Validation Mock Test", () => {
  let languageService: ApexOSLanguageService;
...
group:
  test_group:
    entities:
      - service: apexos.turn_off    # This exists
...
`;
    ...
    const actionDiagnostics = diagnostics.filter(d => 
      d.source === "apexos" && d.code === "unknown-action"
    );
    ...
    const knownActions = ["light.turn_on", "switch.turn_on", "apexos.turn_off"];
