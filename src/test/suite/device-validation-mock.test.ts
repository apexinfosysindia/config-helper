...
// Mock ApexOS connection with predefined devices
...
`;
    ...
    assert.strictEqual(deviceDiagnostics[0].message, "Device 'unknown_device' does not exist in your ApexOS instance");
...
`;
    ...
    const deviceDiagnostics = diagnostics.filter(d => d.code === "unknown-device");
    assert.strictEqual(deviceDiagnostics.length, 1);
    assert.strictEqual(deviceDiagnostics[0].message, "Device 'unknown_device_alt' does not exist in your ApexOS instance");
  });
