...
suite("Entity Validation", () => {
  ...
  suiteSetup(async () => {
    ...
    await vscode.extensions.getExtension("apexos.apexos")?.activate();
  ...
  test("Entity validation file can be opened", async () => {
    ...
    assert.ok(
      ...
      `Document is recognized as YAML or ApexOS (got: ${document.languageId})`
    );
    ...
    // Note: In a real test environment without ApexOS connection,
