...
suite("ApexOS Workspace Detection Tests", () => {
  
  test("Should detect valid ApexOS workspace with .storage folder", async () => {
    ...
    // The isApexOSWorkspace function should return true when:
    // 1. configuration.yaml exists
    // 2. .storage folder exists next to it
    // 3. OR configuration.yaml contains 'apexos:' key
  ...
  test("Should NOT detect non-HA workspace with generic configuration.yaml", async () => {
    // The isApexOSWorkspace function should return false when:
    // 1. configuration.yaml exists but has no HA indicators
    // 2. No .storage folder
    // 3. No other HA-specific files
    // 4. No 'apexos:' key in configuration.yaml
