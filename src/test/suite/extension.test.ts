...
suite("Extension Test Suite", () => {
  suiteSetup(async () => {
    ...
    await vscode.extensions.getExtension("apexos.apexos-config-helper")?.activate();
    
    // Show a message to indicate tests are starting
    vscode.window.showInformationMessage("Starting ApexOS extension tests...");
  });
  ...
  test("Extension is activated", async () => {
    const extension = vscode.extensions.getExtension("apexos.apexos-config-helper");
  ...
  test("Commands are registered", async () => {
    ...
    const extensionCommands = commands.filter(cmd => cmd.startsWith("apexos."));
  ...
  test("Language server activates for ApexOS files", async () => {
    // Open a ApexOS YAML file
    ...
    assert.ok(document.languageId === "yaml" || document.languageId === "apexos", 
