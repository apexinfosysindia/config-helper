...
// Mock ApexOS connection with predefined areas
...
`;
    ...
    const areaDiagnostics = Array.from(new Set(
      diagnostics
        .filter(d => d.source === "apexos" && d.code === "unknown-area")
...
`;
    ...
    const areaDiagnostics = diagnostics.filter(d => 
      d.source === "apexos" && d.code === "unknown-area"
    );
    ...
    const allDiagnostics = diagnostics.filter(d => d.source === "apexos");
