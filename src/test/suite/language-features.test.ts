...
// Tests for ApexOS language features
suite("ApexOS Language Features", () => {
  let workspacePath: string;
  
  suiteSetup(async () => {
    workspacePath = vscode.workspace.workspaceFolders?.[0].uri.fsPath || "";
    vscode.window.showInformationMessage("Starting ApexOS language feature tests...");
  });
  
  test("Configuration file is recognized as ApexOS", async () => {
    ...
    assert.ok(
      document.languageId === "yaml" || document.languageId === "apexos", 
