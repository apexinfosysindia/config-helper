...
export class ApexOSConfiguration {
  ...
  public updateFile = async (uri: string): Promise<FileUpdateResult> => {
    ...
    const apexosYamlFile = new ApexOSYamlFile(
      this.fileAccessor,
      filename,
      ourFile.path,
    );
    this.files[filename] = apexosYamlFile;

    const validationResult = await apexosYamlFile.isValid();
  ...
  private discoverCore = async (
    ...
    try {
      includes = await apexosYamlFile.getIncludes();
    } catch (err) {
      error = true;
...
export interface FilesCollection {
  [filename: string]: ApexOSYamlFile;
}
