...
export function createSocket(
  ...
  console.log(
    "[Auth phase] Initializing WebSocket connection to ApexOS",
    url,
  );
  ...
  function connect(
    ...
    console.log(
      `[Auth Phase] Connecting to ApexOS... Tries left: ${triesLeft}`,
      url,
    );
    ...
    const closeMessage = (ev: {
      ...
      if (ev && ev.code && ev.code !== 1000) {
        errorMessage = `WebSocket connection to ApexOS closed with code ${ev.code} and reason ${ev.reason}`;
      }
    ...
    const errorMessage = (ev: {
      ...
      let errMessage =
        "Disconnected from ApexOS with a WebSocket error";
      if (ev.message) {
    ...
    const closeOrError = (errorText?: string) => {
      if (errorText) {
        console.log(
          `WebSocket Connection to ApexOS closed with an error: ${errorText}`,
        );
      }
      if (invalidAuth) {
    ...
    const handleMessage = (event: {
      ...
      switch (message.type) {
        case MSG_TYPE_AUTH_INVALID:
          invalidAuth = true;
          console.error("[Auth phase] Authentication invalid - token was rejected by ApexOS server");
              ...
              try {
                ...
                if (decodedPayload.exp) {
                  ...
                  if (expiryDate < now) {
                    ...
                    console.error("[Auth phase] Please generate a new token in ApexOS");
                  } else {
