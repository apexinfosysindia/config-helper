...
export class AuthMiddleware {
  ...
  private static getOutputChannel(): vscode.OutputChannel {
    if (!AuthMiddleware.outputChannel) {
      AuthMiddleware.outputChannel = vscode.window.createOutputChannel("ApexOS Auth");
    }
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          settings["apexos"].configTimestamp = Date.now();
      ...
      for (const item of result) {
        if (item && typeof item === "object" && "apexos" in item) {
          ...
          if (token) {
            ...
            item["apexos"].longLivedAccessToken = token;
          ...
          if (fallbackUrl && !item["apexos"].hostUrl) {
            item["apexos"].hostUrl = fallbackUrl;
  ...
  private async handleDidChangeConfigurationNotification(
    ...
    if (params.length > 0) {
      ...
      if (notification && notification.settings) {
        // Ensure the apexos settings object exists
        if (!notification.settings["apexos"]) {
          notification.settings["apexos"] = {};
          AuthMiddleware.log("Created missing apexos settings object in notification");
        }
        ...
        if (token) {
          ...
          notification.settings["apexos"].longLivedAccessToken = token;
          
          // Inject URL if available
          if (url) {
            AuthMiddleware.log(`Injecting URL (${url}) into notification`);
            notification.settings["apexos"].hostUrl = url;
