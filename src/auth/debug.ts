...
/**
 * Debug the authentication (token and ApexOS instance URL) and connection settings
...
export async function debugAuthSettings(context: vscode.ExtensionContext): Promise<void> {
  ...
  debugPanel.clear();
  debugPanel.appendLine("ApexOS Connection Status:");
  ...
  if (activeUrl) {
    ...
    debugPanel.appendLine("No active URL found. Please configure the ApexOS instance URL.");
  }
  ...
  if (activeToken) {
    ...
    if (activeToken.startsWith("eyJ")) {
      ...
      try {
        ...
        if (payload.exp) {
          ...
          if (isExpired) {
            debugPanel.appendLine("⚠️ WARNING: Token has expired! Please generate a new long-lived access token in ApexOS.");
          } else {
    ...
    debugPanel.appendLine("No active token found. Please configure the ApexOS access token.");
  }
  ...
  if (activeUrl && activeToken) {
    ...
    if (ignoreCertificates) {
      debugPanel.appendLine("Ignoring TLS certificate errors (apexos.ignoreCertificates is true).");
    }
    ...
    try {
      ...
      request.on("error", (err) => {
        ...
        if (err.message.includes("CERT_HAS_EXPIRED")) {
          debugPanel.appendLine("Hint: The TLS certificate for your ApexOS instance may have expired.");
        } else if (err.message.includes("UNABLE_TO_VERIFY_LEAF_SIGNATURE")) {
          debugPanel.appendLine("Hint: The TLS certificate for your ApexOS instance is not trusted. You may need to set `apexos.ignoreCertificates` to true in your VS Code settings if you are using a self-signed certificate.");
        } else if (err.message.includes("ENOTFOUND")) {
          debugPanel.appendLine("Hint: The hostname for your ApexOS instance could not be resolved. Check your DNS settings and the URL.");
        } else if (err.message.includes("ECONNREFUSED")) {
          debugPanel.appendLine("Hint: The connection to your ApexOS instance was refused. Ensure ApexOS is running and accessible at the specified URL and port.");
        }
        debugPanel.show();
      });
    ...
    debugPanel.appendLine("Cannot perform connection test: ApexOS instance URL is not configured.");
    debugPanel.show();
  } else {
    debugPanel.appendLine("Cannot perform connection test: ApexOS access token is not configured.");
    debugPanel.show();
  }
}

/**
 * Test connection to ApexOS API
 * @param url The URL to connect to
 * @param token The token to use for authentication
 * @param ignoreCertificates Whether to ignore certificate errors
 * @returns Connection test results
 */
export async function testApexOSConnection(
  url: string, 
  token: string, 
        ...
        res.on("end", () => {
          if (res.statusCode === 200) {
            try {
              ...
              // Extract version information from the ApexOS API response
              ...
              if (parsedData.version) {
                ...
                // Sometimes it's in a welcome message like "API running. ApexOS 2022.5.3"
                const versionMatch = parsedData.message.match(/ApexOS (\d+\.\d+\.\d+)/i);
