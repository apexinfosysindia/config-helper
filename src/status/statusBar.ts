...
import { testApexOSConnection } from "../auth/debug";

/**
 * Status bar item for showing ApexOS connection status
 * Shows the connected instance name, or "Connect" if no connection is available
 */
export class ApexOSStatusBar {
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    this.statusBarItem.command = "apexos.manageAuth";
  ...
  public async checkConnectionStatus(): Promise<void> {
    ...
    try {
      ...
      // Test connection to ApexOS
      const response = await testApexOSConnection(url, token, ignoreCertificates);
      
      if (response.success) {
        // Extract instance name from connection response data if available
        let instanceName = "ApexOS";
        if (response.data) {
      ...
      console.error("Error checking ApexOS connection status:", error);
      this.updateStatus("error");
    } finally {
  ...
  /**
   * Open the ApexOS instance in a browser
   */
  public async openInBrowser(): Promise<void> {
    try {
      ...
      if (url) {
        ...
        vscode.window.showErrorMessage("ApexOS URL is not configured. Please set it first.");
        // Show the authentication management dialog if URL is not set
        vscode.commands.executeCommand("apexos.manageAuth");
      }
    } catch (error) {
      console.error("Error opening ApexOS in browser:", error);
      vscode.window.showErrorMessage(`Failed to open ApexOS: ${error.message || "Unknown error"}`);
    }
  }
  ...
  private updateStatus(status: "connected" | "disconnected" | "error"): void {
    ...
    switch (status) {
      case "connected":
        ...
        this.statusBarItem.tooltip = "Click to open ApexOS in browser";
        this.statusBarItem.command = "apexos.openInBrowser";
        break;
      case "disconnected":
        this.statusBarItem.text = "$(home) Connect";
        this.statusBarItem.tooltip = "Connect to ApexOS";
        this.statusBarItem.command = "apexos.manageAuth";
        break;
      case "error":
        this.statusBarItem.text = "$(home) Error";
        this.statusBarItem.tooltip = "Error connecting to ApexOS";
        this.statusBarItem.command = "apexos.manageAuth";
        break;
    }
  }
