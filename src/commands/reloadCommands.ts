...
export async function showReloadIntegrations(commandMappings: CommandMappings[]): Promise<void> {
  try {
    ...
    const reloadCommands = commandMappings
      ...
      .filter(cmd => cmd.commandId !== "apexos.reloadAll") // Filter out "reloadAll" since it's available in the main menu
      .map(cmd => {
        // Extract just the integration name (domain) from the command ID
        // For example, extract "script" from "apexos.scriptReload"
        const domainName = cmd.domain === "apexos" ? "Core" : cmd.domain;
        
        return {
          label: cmd.commandId.replace("apexos.", ""),
